...
**口径铁律**
1. **商品卡 ≠ 直播**：与直播 campaign `1870075962758033` 是两套投放，**不可相加**。概览表第 3 行只放直播口径。
...
- 🔴🔴 **判定口径固定为下面 5 条链接。不要再跑「未映射清单」，也不要因为上期报告的待办建议就自己扩清单。** 其余一律汇总成一行「其他链接」，不逐条展开；红系/其他品线的消长一律用「按品线汇总」承载。

| 商品ID | 简称 |
|---|---|
| 1734360557016744985 | 白3（精华、霜、水） |
| 1734336273358619673 | 白 single |
| 1732293821262365721 | 白7 |
| 1736359308434310169 | 白3（霜、精华、素颜霜） |
| 1731577149049636889 | 白2 |

- 🟡 **清单外的大链只在结论里备注，不要为它们加行**。
- **白3 有两条链接**，表里各列一行，另加一行「白3 双链合计」。
...
10. **竞品/主播找 KANS**：店铺名 = `Kans Official Vietnam`；业绩表主播用中文名列。
11. **🔴 越南数字格式（最坑）**：点`.`=千分位、逗号`,`=小数。`15.392.399`=15,392,399；`4,13`=4.13。统一解析 `float(re.sub(r'[%\s₫]','',s).replace('.','').replace(',','.'))`。出数后先对总量核量级。
